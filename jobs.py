...
from calibre_plugins.textdiff.textdiff import doMerge, doUnMerge
...
def do_textdiff_bg(args,
                cpus,
                notification=lambda x,y:x):
    # logger.debug("do_textdiff_bg(%s,%s)"%(args,cpus))
    ...
    for j in range(0,len(args['inputepubfns'])):
        ...
        try:
            ...
            if container.opf_version_parsed.major >= 3:
                ...
                ebook_convert_cli_main(['textdiff calling convert',fn,fn2,'--epub-version=2','--no-default-epub-cover','--output-profile=tablet'])
